...
static void ShowCudaProperties(bool showAll, bool showCapabilities, bool showTexture, bool showDeviceTopology, bool hideBanner) {
    ...
    for(var deviceId = 0; deviceId < deviceCount; ++deviceId) {
        ...
        if(showCapabilities || showAll) {
            ...
            WriteLine($"* Memory Decompress Algorithm Mask: {info.MemDecompressAlgorithmMask}");
            WriteLine($"* Memory Decompress Maximum Length: {info.MemDecompressMaximumLength}");
            WriteLine($"* Memory Pool Supported Handle Types: {info.MempoolSupportedHandleTypes}");
            WriteLine($"* Memory Sync Domain Count: {info.MemSyncDomainCount}");
